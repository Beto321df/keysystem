...
-- V2: permite enviar body JSON en POST sin cambiar el resto del sistema.
local function apiRequest(path, method, body)
    ...
    if body ~= nil then
        ...
        local encodedSuccess, encodedBody = pcall(function()
...
GetLinkBtn.MouseButton1Click:Connect(function()
    ...
    if not requestFunc then
        updateStatus(
            "Tu executor no soporta peticiones HTTP",
            Color3.fromRGB(239, 68, 68),
            3
        )
        return
    ...
    GetLinkBtn.Text = "  Generando..."

    updateStatus(
        "Generando enlace con tu dispositivo...",
        Color3.fromRGB(99, 102, 241)
    )
    ...
    task.spawn(function()
        ...
        local copied = pcall(function()
            setclipboard(tostring(data.url))
        end)
        ...
        updateStatus(
            "Enlace seguro copiado con tu dispositivo",
