...
local IMAGE_URL = "https://raw.githubusercontent.com/Fernando143j/system-key/heads/main/950141079_1788854830242953.jpg"
local ICON_KEY_BOX = "https://raw.githubusercontent.com/Fernando143j/system-key/heads/main/Login-Key-1--Streamline-Ultimate.png"
local ICON_VERIFY = "https://raw.githubusercontent.com/Fernando143j/system-key/heads/main/Logout-2--Streamline-Ultimate.png"
local ICON_GETKEY = "https://raw.githubusercontent.com/Fernando143j/system-key/heads/main/Login-Keys--Streamline-Ultimate.png"
local ICON_TITLE = "https://raw.githubusercontent.com/Fernando143j/system-key/heads/main/Programming-Browser--Streamline-Ultimate.png"
local ICON_DISCORD = "https://raw.githubusercontent.com/Fernando143j/system-key/heads/main/discord-white-icon.png"
...
-- HWID
-- =========================================================
-- El backend/website usa HWID- + 24 caracteres hexadecimales.
-- Roblox/RbxAnalyticsService normalmente devuelve un UUID; lo
-- normalizamos de forma determinista para que Lua y la web usen
-- exactamente el mismo deviceId.
...
local function normalizeHwid(value)
    ...
    -- Fallback estable si el executor entrega un identificador sin
    -- suficientes caracteres hexadecimales.
    local h = 2166136261
    for i = 1, #s do
    ...
    local out = string.format("%08X", h)
    local seed = s ~= "" and s or "ZNEXUS"
    while #out < 24 do
        local n = #out + 1
        local c = string.byte(seed, ((n - 1) % #seed) + 1)
        h = bit32.bxor(h, c or 0)
        h = (h * 16777619) % 4294967296
        out = out .. string.format("%08X", h)
    end

    return "HWID-" .. out:sub(1, 24)
end
...
local KEY_WEBSITE_URL = WEBSITE_URL .. "?hwid=" .. urlEncode(deviceId)
...
local function apiRequest(path, method)
    ...
    if not response.Body then
        return false, nil, "El servidor no devolvió una respuesta."
    end
...
if assetFunc and isfile and isfile(folderName .. "/background.jpg") then
    pcall(function()
        BackgroundImage.Image = assetFunc(folderName .. "/background.jpg")
    end)
end
...
if assetFunc and isfile and isfile(folderName .. "/discord.png") then
    pcall(function()
        DiscordBtn.Image = assetFunc(folderName .. "/discord.png")
    end)
end
...
if assetFunc and isfile and isfile(folderName .. "/title_icon.png") then
    pcall(function()
        TitleIcon.Image = assetFunc(folderName .. "/title_icon.png")
    end)
end
...
if assetFunc and isfile and isfile(folderName .. "/key_box.png") then
    pcall(function()
        BoxIcon.Image = assetFunc(folderName .. "/key_box.png")
    end)
end
...
KeyBox.Focused:Connect(function()
    TweenService:Create(PlaceholderLabel, TweenInfo.new(0.2), {TextTransparency = 1}):Play()
end)

KeyBox.FocusLost:Connect(function()
    if KeyBox.Text == "" then
        TweenService:Create(PlaceholderLabel, TweenInfo.new(0.2), {TextTransparency = 0}):Play()
    end
end)
...
if assetFunc and isfile and isfile(folderName .. "/verify.png") then
    pcall(function()
        VerifyIcon.Image = assetFunc(folderName .. "/verify.png")
    end)
end
...
if assetFunc and isfile and isfile(folderName .. "/getkey.png") then
    pcall(function()
        GetKeyIcon.Image = assetFunc(folderName .. "/getkey.png")
    end)
end
...
local function updateStatus(text, color, duration)
    ...
    if duration then
        currentStatusThread = task.delay(duration, function()
            StatusLabel.Text = "2026"
            TweenService:Create(StatusLabel, TweenInfo.new(0.3), {TextColor3 = Color3.fromRGB(130, 130, 160)}):Play()
            currentStatusThread = nil
        end)
    end
end
...
applyHover(VerifyBtn, Color3.fromRGB(99, 102, 241), Color3.fromRGB(115, 118, 255))
applyHover(GetLinkBtn, Color3.fromRGB(24, 24, 33), Color3.fromRGB(35, 35, 48))
...
DiscordBtn.MouseButton1Click:Connect(function()
    if setclipboard then
        ...
        updateStatus("¡Enlace de Discord copiado!", Color3.fromRGB(114, 137, 218), 2)
    else
        updateStatus("Tu executor no soporta setclipboard", Color3.fromRGB(239, 68, 68), 2)
    end
end)
...
GetLinkBtn.MouseButton1Click:Connect(function()
    if setclipboard then
        -- IMPORTANTE: se manda el mismo HWID que usará /api/key/validate.
        -- Así la key nueva queda ligada al executor y no al navegador.
        setclipboard(KEY_WEBSITE_URL)

        GetLinkBtn.Text = "  ¡Copiado!"

        updateStatus("Enlace copiado con tu HWID", Color3.fromRGB(34, 197, 94), 2)
...
VerifyBtn.MouseButton1Click:Connect(function()
    ...
    if userKey == "" then
        updateStatus("Por favor escribe o pega una key", Color3.fromRGB(239, 68, 68), 2)
        return
    end

    if not requestFunc then
        updateStatus("Error: Executor no soporta peticiones HTTP", Color3.fromRGB(239, 68, 68), 3)
        return
    end

    -- Formato NUEVO: FREE_XXXXXXXXX-0000
    if not userKey:match(KEY_PATTERN) then
        VerifyBtn.Text = "  Verificar Key"
        updateStatus("Formato de key inválido", Color3.fromRGB(239, 68, 68), 2)
        return
    end

    VerifyBtn.Text = "  Comprobando..."
    updateStatus("Verificando key con Z Nexus...", Color3.fromRGB(99, 102, 241))

    task.spawn(function()
        local currentDeviceId = deviceId

        if not currentDeviceId or currentDeviceId == "" then
            VerifyBtn.Text = "  Verificar Key"
            updateStatus("No se pudo obtener el HWID", Color3.fromRGB(239, 68, 68), 3)
            return
        end
        ...
        local verifyPath =
            ...
            .. "&deviceId=" .. urlEncode(currentDeviceId)

        local success, data, errorMessage = apiRequest(verifyPath, "GET")

        if not success then
            VerifyBtn.Text = "  Verificar Key"
            updateStatus(errorMessage or "Error de conexión con el servidor", Color3.fromRGB(239, 68, 68), 3)
            return
        end
        ...
        if not data.valid then
            ...
            if data.expired then
                updateStatus("Esta key ya ha expirado", Color3.fromRGB(239, 68, 68), 3)
            elseif data.reason == "device_mismatch" then
                updateStatus("Esta key pertenece a otro dispositivo", Color3.fromRGB(239, 68, 68), 3)
            else
                updateStatus("Key inválida o vinculada a otro dispositivo", Color3.fromRGB(239, 68, 68), 3)
            end
        ...
        updateStatus("¡Key verificada correctamente!", Color3.fromRGB(34, 197, 94))
        ...
        if SCRIPT_LOADER_URL and SCRIPT_LOADER_URL ~= "" then
            local loadSuccess, loadErr = pcall(function()
                loadstring(game:HttpGet(SCRIPT_LOADER_URL))()
            end)

            if not loadSuccess then
                warn("[Z NEXUS ERROR] Fallo al cargar el Main Script:", loadErr)
            end
        end
    end)
end)
