...
async function recoverFromTicket(database,ticketHash,sessionId,deviceId,link,userAgentHash,acceptLanguageHash){
  if(!ticketHash)return null;
  const ref=database.ref(`sessionTickets/${ticketHash}`),snap=await ref.get();
  if(!snap.exists())return null;
  const t=snap.val()||{};
  if(t.used===true||Number(t.expiresAt||0)<=Date.now()){await ref.remove();return null;}
  if(String(t.sessionId||'')!==sessionId||String(t.deviceId||'')!==deviceId||Number(t.link)!==link)return null;
  if(t.userAgentHash&&!equal(String(t.userAgentHash),userAgentHash))return null;
  if(t.acceptLanguageHash&&!equal(String(t.acceptLanguageHash),acceptLanguageHash))return null;
  const required=REQUIREMENTS[Number(t.dur)];if(!required)return null;
  const session={id:sessionId,dur:Number(t.dur),link:Number(t.link),state:'awaiting_external_return',createdAt:Number(t.createdAt||Date.now()),expiresAt:Number(t.expiresAt),deviceId,completedLinks:Math.max(0,Number(t.completedLinks||0)),attempts:1,verificationUsed:false,ticketHash,ticketIssuedAt:Number(t.createdAt||Date.now()),ticketExpiresAt:Number(t.expiresAt),ticketUserAgentHash:String(t.userAgentHash||''),ticketAcceptLanguageHash:String(t.acceptLanguageHash||''),ticketProviderHost:String(t.providerHost||''),verificationNonce:crypto.randomBytes(24).toString('hex')};
...
module.exports=async(req,res)=>{try{
  ...
  const database=db(),ref=database.ref(`sessions/${sessionId}`),ticket=cookie(req,'__Host-znexus_ticket'),ticketHash=ticket?sha256(ticket):'',userAgentHash=sha256(req.headers?.['user-agent']||''),acceptLanguageHash=sha256(req.headers?.['accept-language']||'');
  let snap=await ref.get();
  if(!snap.exists()){
    const restored=await recoverFromTicket(database,ticketHash,sessionId,deviceId,link,userAgentHash,acceptLanguageHash);
  ...
  const tx=await ref.transaction(current=>{
    ...
    if(String(current.deviceId||'')!==deviceId)return;
    if(current.state!=='awaiting_external_return'||Number(current.link)!==link)return;
    if(current.verificationUsed===true)return;
    if(!current.ticketHash||!equal(String(current.ticketHash),ticketHash))return;
    if(Number(current.ticketExpiresAt||0)<=Date.now())return;
    if(current.ticketUserAgentHash&&!equal(String(current.ticketUserAgentHash),userAgentHash))return;
    if(current.ticketAcceptLanguageHash&&!equal(String(current.ticketAcceptLanguageHash),acceptLanguageHash))return;
    current.link=next;
    current.completedLinks=completed;
    current.totalLinks=required;
    current.state=state;
    ...
    current.ticketHash=null;
    current.ticketIssuedAt=null;
    current.ticketExpiresAt=null;
    current.ticketUserAgentHash=null;
    current.ticketAcceptLanguageHash=null;
    current.ticketProviderHost=null;
    current.externalStartedAt=null;
  ...
  return json(res,200,{session:{id:out.id,dur:Number(out.dur),link:Number(out.link),state:out.state,createdAt:Number(out.createdAt),expiresAt:Number(out.expiresAt)}});
