...
async function turnstile(token,req){
  ...
  if(String(d.action||'').trim()&&String(d.action).trim()!=='keyverify')throw Object.assign(new Error('La verificación no corresponde a este acceso.'),{status:403});
  if(HOSTNAME&&Array.isArray(d.hostname)&&d.hostname.length===0)throw Object.assign(new Error('Host de verificación inválido.'),{status:403});
  if(HOSTNAME&&d.hostname&&String(d.hostname).trim().toLowerCase()!==HOSTNAME)throw Object.assign(new Error('Host de verificación inválido.'),{status:403});
  return d;
}
...
module.exports=async(req,res)=>{try{
  ...
  return json(res,200,{session:{id:saved.val().id,dur:Number(saved.val().dur),link:Number(saved.val().link),state:saved.val().state,createdAt:Number(saved.val().createdAt),expiresAt:Number(saved.val().expiresAt)}});
