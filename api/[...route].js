...
function parseServiceAccount(raw){let s=String(raw||'').trim();const candidates=[s];if((s.startsWith('"')&&s.endsWith('"'))||(s.startsWith("'")&&s.endsWith("'"))){try{const q=s.startsWith('"')?JSON.parse(s):s.slice(1,-1);if(typeof q==='string')candidates.unshift(q)}catch{}}for(const candidate of candidates){try{let o=JSON.parse(candidate);if(typeof o==='string')o=JSON.parse(o);if(o&&typeof o==='object'&&o.project_id&&o.client_email&&o.private_key)return o}catch{}}const start=s.indexOf('{');if(start>=0){let depth=0,inString=false,escaped=false;for(let i=start;i<s.length;i++){const c=s[i];if(inString){if(escaped)escaped=false;else if(c==='\\')escaped=true;else if(c==='"')inString=false;continue}if(c==='"'){inString=true;continue}if(c==='{')depth++;else if(c==='}'&&--depth===0){try{const o=JSON.parse(s.slice(start,i+1));if(o&&o.project_id&&o.client_email&&o.private_key)return o}catch{}break}}}throw new Error('FIREBASE_SERVICE_ACCOUNT_JSONZ no contiene un JSON de Service Account válido.');}
...
const origin=String(req.headers?.origin||'');if(origin==='https://zkeysystem.vercel.app'||origin==='null'){res.setHeader('Access-Control-Allow-Origin',origin);res.setHeader('Vary','Origin')}res.setHeader('Access-Control-Allow-Methods','GET,POST,OPTIONS');res.setHeader('Access-Control-Allow-Headers','Content-Type, Accept');
