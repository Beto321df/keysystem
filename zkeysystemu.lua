...
VerifyBtn.MouseButton1Click:Connect(function()
    ...
    task.spawn(function()
        ...
        if not success then
            VerifyBtn.Text = "  Verificar Key"
            updateStatus(
                errorMessage or "Error de conexiÃ³n con el servidor",
                Color3.fromRGB(239, 68, 68),
                3
            )
            return
        ...
        -- =================================================
        -- KEY INVÃLIDA / FALLBACK RECOVER
        -- =================================================

        if data.valid ~= true then

            -- Si /validate no reconoce la key directamente,
            -- consultamos la key que el servidor tiene asociada
            -- al mismo HWID.
            local recoverPath =
                "/api/key/recover"
                .. "?deviceId=" .. urlEncode(deviceId)

            local recoverSuccess, recovered, recoverError =
                apiRequest(
                    recoverPath,
                    "GET"
                )
            ...
            if recoverSuccess
                ...
                and recovered.found == true then

                local serverKey =
                    tostring(recovered.key or "")
                    :gsub("^%s+", "")
                    :gsub("%s+$", "")
                    :upper()
                ...
                if serverKey == userKey then
                    ...
                    -- El servidor confirmÃ³ que esta key pertenece
                    -- al mismo HWID.
                    data.valid = true
                    data.recovered = true
                    ...
                    warn(
                        "[Z NEXUS DEBUG] Key no coincide | servidor=",
                        serverKey,
                        "| ingresada=",
                        userKey,
                        "| hwid=",
            ...
            elseif recovered
                and type(recovered) == "table"
                and recovered.expired == true then
            ...
            -- Si tampoco recuperamos una key vÃ¡lida,
            -- conserva el comportamiento de error original.
            if data.valid ~= true then
                ...
                elseif data.reason == "not_found" then
                    ...
                    warn(
                        "[Z NEXUS DEBUG] Key invÃ¡lida | reason=",
                        reason,
                        "| hwid=",
                        tostring(deviceId),
                        "| key=",
                        tostring(userKey),
                        "| recover=",
                        recoverSuccess and "respuesta" or "error"
                    )

                    updateStatus(
                        "Key invÃ¡lida | " .. reason,
                        Color3.fromRGB(239, 68, 68),
                        4
                    )
                end
        ...
        VerifyBtn.Text = "  Â¡Acceso Concedido!"
        ...
        updateStatus(
            "Â¡Key verificada correctamente!",
            Color3.fromRGB(34, 197, 94)
        )
        ...
        if SCRIPT_LOADER_URL
            ...
            local loadSuccess, loadErr = pcall(function()
                ...
                if not source or source == "" then
                    error("El Main Script estÃ¡ vacÃ­o.")
                end
